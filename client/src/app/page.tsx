    ...
    <div className="flex flex-1 flex-col">
      ...
      <main className="flex-1">
        <section className="relative overflow-hidden">
          <div className="pointer-events-none absolute inset-x-0 top-0 -z-10 h-128 bg-[radial-gradient(ellipse_at_top,rgba(139,92,246,0.16),transparent_60%)]" />
          ...
          <div className="mx-auto grid w-full max-w-6xl items-center gap-14 px-6 pt-16 pb-20 lg:grid-cols-2 lg:pt-24">
            <div>
              ...
              <h1 className="mt-6 text-4xl font-semibold tracking-tight text-zinc-50 sm:text-5xl lg:text-6xl">
                Watch together,
                <br />
                <span className="bg-linear-to-r from-violet-400 via-fuchsia-400 to-violet-300 bg-clip-text text-transparent">
                  perfectly in sync.
                </span>
              </h1>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <Link
                  href="/join"
                  className="flex items-center justify-center gap-2 rounded-xl border border-zinc-700 px-5 py-3 text-sm font-medium text-zinc-200 transition hover:bg-zinc-800"
                >
            ...
            <div className="relative">
              <div className="overflow-hidden rounded-2xl border border-zinc-700 bg-zinc-900 shadow-2xl shadow-black/50">
                <div className="flex items-center gap-2 border-b border-zinc-800 px-4 py-3">
                  <span className="flex h-6 w-6 items-center justify-center rounded-md bg-violet-600 text-white">
                    <Video className="h-3 w-3" />
                  </span>
                  <span className="text-xs font-medium text-zinc-50">Friday movie night</span>
                  <span className="ml-auto flex items-center gap-1.5 rounded-full border border-zinc-700 px-2 py-0.5 text-[11px] text-zinc-400">
                    <Users className="h-3 w-3" />3
                  </span>
                </div>

                <div className="grid sm:grid-cols-[1fr_9rem]">
                  <div className="relative aspect-video bg-[linear-gradient(135deg,#4c1d95,#1a0b33)]">
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="flex h-12 w-12 items-center justify-center rounded-full bg-white text-violet-900">
                        <Play className="ml-0.5 h-5 w-5" fill="currentColor" />
                      </span>
                    </div>
                    <div className="absolute inset-x-0 bottom-0 px-4 pb-3">
                      <div className="h-1 w-full rounded-full bg-white/20">
                        <div className="h-1 w-2/5 rounded-full bg-violet-400" />
                      </div>
                      <div className="mt-1.5 flex justify-between text-[10px] text-zinc-300 tabular-nums">
                        <span>41:12</span>
                        <span>1:43:00</span>
                      </div>
                    </div>
                  ...
                  <div className="hidden flex-col gap-2 border-l border-zinc-800 bg-zinc-900/60 p-3 sm:flex">
                    {chatPreview.map((m) => (
                      <div key={m.name} className="text-[11px]">
                        <p className="font-medium text-zinc-200">{m.name}</p>
                        <p className="text-zinc-500">{m.text}</p>
                      </div>
                    ))}
                    <div className="mt-auto h-6 rounded-md border border-zinc-800 bg-zinc-900/50" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {features.map(({ icon: Icon, title, body }) => (
              <div key={title} className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6">
                <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-violet-500/10 text-violet-400 ring-1 ring-violet-500/20">
                  <Icon className="h-4 w-4" />
                </span>
                <h2 className="mt-4 text-sm font-semibold text-zinc-50">{title}</h2>
        ...
        <section className="mx-auto w-full max-w-6xl px-6 py-16">
          <div className="mb-10 max-w-md">
            ...
            <p className="mt-2 text-sm text-zinc-400">Three steps, under a minute.</p>
          </div>
          <ol className="grid gap-8 md:grid-cols-3">
            {steps.map((s) => (
              <li key={s.n} className="border-t border-zinc-700 pt-5">
                <span className="font-mono text-xs text-violet-400">{s.n}</span>
                <h3 className="mt-2 text-sm font-semibold text-zinc-50">{s.title}</h3>
        ...
        <section className="mx-auto w-full max-w-6xl px-6 pt-8 pb-24">
          <div className="flex flex-col items-start gap-6 rounded-2xl border border-violet-500/30 bg-linear-to-br from-violet-700 via-violet-800 to-violet-950 p-8 text-white shadow-2xl shadow-violet-900/40 sm:flex-row sm:items-center sm:justify-between sm:p-10">
            <div>
            ...
            <Link
              href="/create"
              className="flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-medium text-violet-900 shadow-lg shadow-black/20 transition hover:bg-violet-100"
            >
              Create a room
              <ArrowRight className="h-4 w-4" />
            </Link>
      ...
      <footer className="border-t border-zinc-800">
        <div className="mx-auto flex w-full max-w-6xl flex-col items-center justify-between gap-3 px-6 py-6 text-xs text-zinc-500 sm:flex-row">
          ...
          <Link href="/about" className="text-zinc-400 underline-offset-4 hover:underline">
            About
          </Link>
        </div>
      </footer>
